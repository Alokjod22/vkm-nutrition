...
export interface Product {
  ...
  category: "Protein" | "Creatine" | "Pre-Workout & BCAA" | "Vitamins & Wellness" | "Peanut Butter" | "Oats & Muesli";
  price: number;
  mrp: number;
  discount: number;
  rating: number;
...
class DataStore {
  ...
  private loadClientData() {
    ...
    try {
      const p = localStorage.getItem("vkm_products_v4");
  ...
  private saveClientData() {
    ...
    try {
      localStorage.setItem("vkm_products_v4", JSON.stringify(this.products));
